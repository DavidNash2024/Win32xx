// Table.cpp: implementation of the FunctionTable
...
namespace Calc
{
	...
	//////////////////////////////////////////////////////////////////////
	// FunctionTable function members
	//////////////////////////////////////////////////////////////////////
	...
	PFun FunctionTable::GetFun(const std::string& funName)
	{
		PFun pFunpointer = 0;
		std::map<std::string, PFun>::iterator it = m_functions.find(funName);
	...
	bool	FunctionTable::IsFunction(const std::string& funName)
	{
	...
	bool SymbolTable::IsSymbol(const std::string& symbol)
	{
		return (m_symbolMap.find(symbol) != m_symbolMap.end());
	}

	// Retrieves the value of the specified symbol.
	double SymbolTable::GetValue(const std::string& symbol) const
	{
		double value = 0;
		std::map<std::string, double>::const_iterator it = m_symbolMap.find(symbol);
	...
	void SymbolTable::SetValue(const std::string& symbol, double value)
	{
		std::map<std::string, double>::iterator it = m_symbolMap.find(symbol);
		if (it != m_symbolMap.end())
			(*it).second = value;
		else
