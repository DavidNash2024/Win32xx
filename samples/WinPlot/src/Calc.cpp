...
#pragma warning ( disable : 26812 )       // enum type is unscoped. 
	...
	Calculator::Calculator(const std::string& buffer)
	...
	void Calculator::Input(const std::string& buffer)
	{
	...
	Node* Calculator::Unit()
	{
		...
		case tFunction:
			...
			if (m_parse.GetToken() == tLeftParenth)
			{
				...
				std::string FunctionName = m_parse.GetAlphaName();
	...
	void Calculator::SetVar(const string& Var, double Val)
	{
	...
	///////////////////////////////////////////////
	Calculator::Parser::Parser(const std::string& buffer)
	...
	eToken Calculator::Parser::GetToken()
	{
		...
		case '\0':
			...
			break;
		default:
			if (isalpha(m_buffer[Index]) || m_buffer[Index] == '_')
			{
				m_alphaName = "";
				int TempIndex = Index;
				do
				{
					m_alphaName += m_buffer[TempIndex++];
				} while (isalnum(m_buffer[TempIndex]));
	...
	bool Calculator::Parser::AcceptToken()
	{
		...
		{
			char *p;
			const char* charArray = m_buffer.c_str();
			double value = strtod(charArray + m_index, &p);
			if (p != charArray)
			{
				m_index = p - charArray;
				m_number = value;
	...
	void Calculator::Parser::Input(const std::string& buffer)
	{
		m_buffer = buffer;
		m_index = 0;
	}
