...
void CDoc::LoadDocRegistry(LPCWSTR keyName)
{
    ...
    if (ERROR_SUCCESS == key.Open(HKEY_CURRENT_USER, strKey, KEY_READ))
    {
        ...
        m_intVal = static_cast<int>(RegQueryDWORDValue(key, L"Int1"));
        m_UINTVal = RegQueryDWORDValue(key, L"UINT1");
        m_longVal = static_cast<LONG>(RegQueryDWORDValue(key, L"Long1"));
        m_ULongVal = RegQueryDWORDValue(key, L"ULong1");

        CString s = RegQueryStringValue(key, L"Float1");
        LPWSTR p = nullptr;
        m_floatVal = static_cast<float>(::wcstod(s.c_str(), &p));
...
{
    return (RegQueryDWORDValue(key, pName) & 1) ? TRUE : FALSE;
}
...
void CDoc::SaveDocRegistry(LPCWSTR keyName)
{
    ...
    if (ERROR_SUCCESS == key.Create(HKEY_CURRENT_USER, strKey, nullptr,
        REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, nullptr))
    {
        ...
        s.Format(L"%.*g", DBL_DIG, m_doubleVal);
        ...
        ULONG size = sizeof(SYSTEMTIME);
        key.SetBinaryValue(L"MyDateTime", static_cast<const void*>(&m_dateSysTime), size);
        key.SetBinaryValue(L"MoCalendar1", static_cast<const void*>(&m_calDateSysTime), size);
    }
}
