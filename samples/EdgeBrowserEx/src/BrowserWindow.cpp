...
HRESULT CBrowserWindow::CreateBrowserControlsWebView()
{
    return m_uiEnv->CreateCoreWebView2Controller(*this, Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
        [this](HRESULT result, ICoreWebView2Controller* host) -> HRESULT
        {
            if (!SUCCEEDED(result))
            {
                Trace(L"Controls WebView creation failed\n");
                return result;
            }
...
HRESULT CBrowserWindow::CreateBrowserOptionsWebView()
{
    return m_uiEnv->CreateCoreWebView2Controller(*this, Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
        [this](HRESULT result, ICoreWebView2Controller* host) -> HRESULT
        {
            if (!SUCCEEDED(result))
            {
                Trace(L"Options WebView creation failed\n");
                return result;
            }
...
HRESULT CBrowserWindow::HandleTabMessageReceived(size_t tabId, ICoreWebView2* webview, ICoreWebView2WebMessageReceivedEventArgs* eventArgs)
{
    ...
    case MG_REMOVE_FAVORITE:     return OnTabFavourites(tabId, webview, jsonObj);

    case MG_GET_SETTINGS:        return OnTabSettings(tabId, webview, jsonObj);
    case MG_CLEAR_CACHE:         return OnTabClearCache(tabId, webview, jsonObj);
    case MG_CLEAR_COOKIES:       return OnTabClearCookies(tabId, webview, jsonObj);

    case MG_GET_HISTORY:
    case MG_REMOVE_HISTORY_ITEM:
    case MG_CLEAR_HISTORY:       return OnTabHistory(tabId, webview, jsonObj);
...
BOOL CBrowserWindow::LaunchBrowser()
{
    ...
    HRESULT hr = CreateCoreWebView2EnvironmentWithOptions(nullptr, userDataDirectory.c_str(),
        nullptr, Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>(
            [this](HRESULT result, ICoreWebView2Environment* env) -> HRESULT
            {
                ...
                if (!SUCCEEDED(hr))
                {
                    Trace(L"UI WebViews environment creation failed\n");
                }
    ...
    if (!SUCCEEDED(hr))
    {
        Trace(L"Content WebViews environment creation failed\n");
        return FALSE;
    }
...
// Called in response to the MG_GET_FAVORITES and MG_REMOVE_FAVORITE messages.
HRESULT CBrowserWindow::OnTabFavourites(size_t tabId, ICoreWebView2* webview, json jsonObj)
{
    wil::unique_cotaskmem_string source;
    RETURN_IF_FAILED(webview->get_Source(&source));
...
// Called in response to the MG_GET_SETTINGS message.
HRESULT CBrowserWindow::OnTabSettings(size_t tabId, ICoreWebView2* webview, json jsonObj)
{
    wil::unique_cotaskmem_string source;
    RETURN_IF_FAILED(webview->get_Source(&source));
...
// Called in response to the MG_CLEAR_CACHE message.
HRESULT CBrowserWindow::OnTabClearCache(size_t tabId, ICoreWebView2* webview, json jsonObj)
{
...
// Called in response to the MG_CLEAR_COOKIES message.
HRESULT CBrowserWindow::OnTabClearCookies(size_t tabId, ICoreWebView2* webview, json jsonObj)
{
    wil::unique_cotaskmem_string uri;
    RETURN_IF_FAILED(webview->get_Source(&uri));
...
// Called in response to the case MG_GET_HISTORY, MG_REMOVE_HISTORY_ITEM, and MG_CLEAR_HISTORY messages.
HRESULT CBrowserWindow::OnTabHistory(size_t tabId, ICoreWebView2* webview, json jsonObj)
{
    wil::unique_cotaskmem_string uri;
    RETURN_IF_FAILED(webview->get_Source(&uri));
...
// Called in response to the MG_CREATETAB message.
HRESULT CBrowserWindow::OnMGCreateTab(json args)
{
...
// Called in response to the MG_NAVIGATE message.
HRESULT CBrowserWindow::OnMGNavigate(json args)
{
    ...
    if (uri.substr(0, browserScheme.size()).compare(browserScheme) == 0)
    {
        ...
        {
            Trace(L"Requested unknown browser page\n");
        }
    }
...
// Called in response to the MG_GOFORWARD message.
HRESULT CBrowserWindow::OnMGGoForward(json)
{
    CheckFailure(m_tabs.at(m_activeTabId)->m_contentWebView->GoForward(), L"");
    return S_OK;
}

// Called in response to the MG_GOBACK message.
HRESULT CBrowserWindow::OnMGGoBack(json)
{
    CheckFailure(m_tabs.at(m_activeTabId)->m_contentWebView->GoBack(), L"");
    return S_OK;
}

// Called in response to the MG_RELOAD message.
HRESULT CBrowserWindow::OnMGReload(json)
{
    CheckFailure(m_tabs.at(m_activeTabId)->m_contentWebView->Reload(), L"");
    return S_OK;
}

// Called in response to the MG_CANCEL message.
HRESULT CBrowserWindow::OnMGCancel(json)
{
    CheckFailure(m_tabs.at(m_activeTabId)->m_contentWebView->CallDevToolsProtocolMethod(L"Page.stopLoading", L"{}", nullptr), L"");
    return S_OK;
}

// Called in response to the MG_SWITCH_TAB message.
HRESULT CBrowserWindow::OnMGSwitchTab(json args)
{
...
// Called in response to the MG_CLOSE_TAB message.
HRESULT CBrowserWindow::OnMGCloseTab(json args)
{
    size_t id = args.at("tabId").get<size_t>();
    m_tabs.at(id)->m_contentController->Close();
    m_tabs.erase(id);
...
// Called in response to the MG_CLOSE_WINDOW message.
HRESULT CBrowserWindow::OnMGCloseWindow(json)
{
    DestroyWindow(*this);
    return S_OK;
}

// Called in response to the MG_SHOW_OPTIONS message.
HRESULT CBrowserWindow::OnMGShowOptions(json)
{
...
// Called in response to the MG_HIDE_OPTIONS message.
HRESULT CBrowserWindow::OnMGHideOptions(json)
{
    CheckFailure(m_optionsController->put_IsVisible(FALSE), L"Something went wrong when trying to close the options dropdown.");
    return S_OK;
}

// Called in response to the MG_OPTION_SELECTED message.
HRESULT CBrowserWindow::OnMGOptionSelected(json)
{
    m_tabs.at(m_activeTabId)->m_contentController->MoveFocus(COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC);
    return S_OK;
}

// Called in response to the MG_GET_FAVORITES, MG_GET_SETTINGS, and MG_GET_HISTORY messages.
HRESULT CBrowserWindow::OnMGHistory(json args, json jsonObj)
{
...
void CBrowserWindow::SetUIMessageBroker()
{
        ...
        [this](ICoreWebView2* webview, ICoreWebView2WebMessageReceivedEventArgs* eventArgs) -> HRESULT
    {
        ...
        {
            Trace(L"No message code provided\n");
            return S_OK;
        }

        if (!jsonObj.contains("args"))
        {
            Trace(L"The message has no args field\n");
            return S_OK;
        }
        ...
        switch (message)
        {
        case MG_CREATE_TAB:      return OnMGCreateTab(args);
        case MG_NAVIGATE:        return OnMGNavigate(args);
        case MG_GO_FORWARD:      return OnMGGoForward(args);
        case MG_GO_BACK:         return OnMGGoBack(args);
        case MG_RELOAD:          return OnMGReload(args);
        case MG_CANCEL:          return OnMGCancel(args);
        case MG_SWITCH_TAB:      return OnMGSwitchTab(args);
        case MG_CLOSE_TAB:       return OnMGCloseTab(args);
        case MG_CLOSE_WINDOW:    return OnMGCloseWindow(args);
        case MG_SHOW_OPTIONS:    return OnMGShowOptions(args);
        case MG_HIDE_OPTIONS:    return OnMGHideOptions(args);
        case MG_OPTION_SELECTED: return OnMGOptionSelected(args);

        case MG_GET_FAVORITES:
        case MG_GET_SETTINGS:
        case MG_GET_HISTORY:     return OnMGHistory(args, jsonObj);
