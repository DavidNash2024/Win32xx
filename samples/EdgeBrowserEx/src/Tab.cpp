...
HRESULT Tab::Init(ICoreWebView2Environment* env, bool shouldBeActive)
{
    return env->CreateCoreWebView2Controller(m_parentHWnd, Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
        ...
        CBrowserWindow::CheckFailure(m_contentController->get_CoreWebView2(&m_contentWebView), L"");
        CBrowserWindow*  browserWindow = reinterpret_cast<CBrowserWindow*>(CWnd::GetCWndPtr(m_parentHWnd));
        ...
        {
            CBrowserWindow::CheckFailure(browserWindow->HandleTabNavCompleted(m_tabId, webview, args), L"Can't udpate reload button");
            return S_OK;
        ...
        CBrowserWindow::CheckFailure(m_contentWebView->GetDevToolsProtocolEventReceiver(L"Security.securityStateChanged", &m_securityStateChangedReceiver), L"");
        ...
        {
            CBrowserWindow::CheckFailure(browserWindow->HandleTabSecurityUpdate(m_tabId, webview, args), L"Can't udpate security icon");
            return S_OK;
...
void Tab::SetMessageBroker()
{
    ...
    {
        CBrowserWindow* browserWindow = reinterpret_cast<CBrowserWindow*>(CWnd::GetCWndPtr(m_parentHWnd));
        CBrowserWindow::CheckFailure(browserWindow->HandleTabMessageReceived(m_tabId, webview, eventArgs), L"");
...
HRESULT Tab::ResizeWebView()
{
    ...
    CBrowserWindow* browserWindow = reinterpret_cast<CBrowserWindow*>(CWnd::GetCWndPtr(m_parentHWnd));
    bounds.top += browserWindow->GetDPIAwareBound(CBrowserWindow::c_uiBarHeight);
