...
// Shared_Ptr (or shared_ptr) for dynamically allocated arrays. See below
// for advice on how to wrap dynamically allocated arrays in a smart object.
//
...
namespace Win32xx
{
	...
	public:
		...
		Shared_Ptr(T1 * p) : m_ptr(p), m_count(p ? new int(0) : NULL) { inc_ref(); }
